...
export default function SetPassword() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const res = await axios.get(`http://localhost:8000/api/auth/profile?token=${token}`);
        setUser(res.data.data);  // set user as res.data.data
      } catch (error) {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      await axios.post(`http://localhost:8000/api/auth/set-pass?token=${token}`, { password, full_name, birth_date });
        ...
        <div>
          {user && (
            <div>
              ...
              <p className='text-center'>Birth Date: {new Date(user.birth_date).toISOString().split('T')[0]}</p>
            </div>
          )}
              ...
              <Form className="relative justify-center">
                ...
                <div className='grid grid-cols-1 mt-7 mb-1 pb-3'>
                  ...
                  <div className='font-Roboto relative mt-6'>
                    ...
                    <Field className='border border-gray-300 h-6 text-xs w-full focus:border-darkgreen focus:ring-0' type={showConfirmPassword ? 'text' : 'password'} name='confirm_password' placeholder='Confirm Password' />
                    <div className="absolute right-1 top-1/2 transform pt-1 -translate-y-1/2">
                  ...
                  <div className='font-Roboto relative mt-4'>
                    ...
                    <Field className='border border-gray-300 h-6 text-xs w-full focus:border-darkgreen focus:ring-0' type='text' name='full_name' placeholder='Full Name (optional)' />
                  </div>
                  <div className='font-Roboto relative mt-4'>
                    <ErrorMessage name='birth_date' component='div' className='text-redd text-xs absolute -top-5' />
                    <Field className='border border-gray-300 h-6 text-xs w-full focus:border-darkgreen focus:ring-0' type='date' name='birth_date' placeholder='Birth Date (optional)' />
                  </div>
                </div>
                <button
