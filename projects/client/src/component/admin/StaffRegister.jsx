...
const StaffRegistrationForm = () => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    join_date: Yup.date()
      .max(new Date(), "Join date can't be in the future")
          ...
          >
            {({ isSubmitting, status }) => (
              <Form className="relative justify-center">
                ...
                {isRegistered && (
                  <p className="text-xs text-center font-Roboto mb-4 text-greenn tracking-wide sm:text-sm">
                ...
                {status && !status.success && (
                  <div className="text-redd text-xs ">{status.message}</div>
                )}
